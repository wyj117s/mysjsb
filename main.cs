...
namespace opencv6
{
    public partial class main : Form
    {
        ...
        /// <summary>
        /// 使用SIFT特征检测器进行特征匹配，初步测试效果不佳，还未调试参数，未使用
        /// </summary>
        /// <param name="img1"></param>
        /// <param name="img2"></param>
        /// <returns></returns>
        //private float MatchImagesUsingSIFT(Mat img1, Mat img2)
        //{
        //    VectorOfKeyPoint keyPoints1 = new VectorOfKeyPoint();
        //    VectorOfKeyPoint keyPoints2 = new VectorOfKeyPoint();
        //    Mat descriptors1 = new Mat();
        ...
        //    _sift.DetectAndCompute(img1, null, keyPoints1, descriptors1, false);
        //    _sift.DetectAndCompute(img2, null, keyPoints2, descriptors2, false);
        ...
        //contrastThreshold：对比度阈值，用于过滤具有较低对比度的特征
        //edgeThreshold：边缘阈值，用于过滤位于边缘上的特征点
        //sigma：高斯模糊的sigma值
        //matchDistanceFactor：用于确定匹配是否为好的匹配，其值乘以第二近邻匹配的距离
        private float MatchImagesUsingSIFT(Mat img1, Mat img2, int nFeatures = 0, int nOctaveLayers = 3, double contrastThreshold = 0.04, double edgeThreshold = 10, double sigma = 1.6, float matchDistanceFactor = 0.75f)
        {
            ...
            foreach (var match in matches.ToArrayOfArray())
            {
                if (match[0].Distance < matchDistanceFactor * match[1].Distance)
                {
                    sumDistances += match[0].Distance;
                    numGoodMatches++;
                }
